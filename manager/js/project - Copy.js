...
$(document).ready(function () {    
    ...
    function ajaxGetFileList(_path,_onlyFolders,_callBack){
        ...
        $.ajax({
            ...
            data: sendData,
            success: function(data){
                console.log('ajaxGetFileList success');
        ...
        if(data.length>0){
            ...
            /*if (PROJECTJS.viewedFolders.length > 0) {
                ...
                for (var i = 0; i < data.length; i++) {
                    $('#target_dir').find('ul').append('<li class="item" data-path="' + data[i].path + '"><span>' + data[i].name + '</span></li>');
                }
    ...
    $('#dialog_select_dir').on('show.bs.modal', function () {
        ...
        ajaxGetDriveList(function(error){
            ...
            ajaxGetFileList($('#current_drive_val').val(),true, function(data){
                displayFileList(data);
            });
            //ajaxGetFileList('c:\\wamp2',true);
        });
    });
