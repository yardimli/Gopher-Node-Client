            ...
            fs.readFile(filePath,function(err,contents){
                if(!err){
					...
					if ( (mimeType=="application/javascript") && (filePath.indexOf("jquery")==-1) )
					{
						...
						fs.writeFile(filePath+".gopher.pure",JSON.stringify(parsed));
						if (socketVar!=0) { 
							socketVar.emit('ParsedGopher',{
								filename:filePath,
...
function startServer(debug)
{
	...
	function onRequest(request, response) 
	{
		...
		if (filePathName.search("/admin/")!=-1)
		{
			filePathName = filePathName.replace("/admin/","/");
			getFileAdmin((localFolderAdmin + filePathName + fileName),response,page404,extensions[ext]);	  
		} else
		{
